    ...
    public boolean onInterceptTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                intercepted = false;
                mLastDownX = (int) event.getY();
//                LogUtil.d(LOG_TAG, "mLastDownX= " + mLastDownX);
                break;
            case MotionEvent.ACTION_MOVE:
                // <!-- Base "touch slop" value used by ViewConfiguration as a
                // <dimen name="config_viewConfigurationTouchSlop">8dp</dimen>
                intercepted = Math.abs(event.getY() - mLastDownX) >= ViewConfiguration.get(
...
//                LogUtil.d(LOG_TAG, "mLastX(ACTION_DOWN)= " + mLastX);
            ...
            case MotionEvent.ACTION_UP:
                ...
                int xVelocity = (int) mVelocityTracker.getYVelocity();
//                LogUtil.d(LOG_TAG, "xVelocity= " + xVelocity);
                releaseVelocityTracker();
                // 总共滑动的距离
                deltaX = x - mLastDownX;
                // 当总共滑动距离超过半屏时/超过最低解锁速率时解锁
                if (deltaX < 0) {//mMinimumVelocity=787
                    if ((Math.abs(deltaX) > mScreenWidth / 3) || -xVelocity > mMinimumVelocity) {
                        smoothScrollTo(getScrollY(), mScreenWidth, DURATION);
                        mCloseFlag = true;

                    } else { // 向右滑动未超过半个屏幕宽的时候 开启向左弹动动画
                        smoothScrollTo(getScrollY(), -getScrollY(), DURATION);
                    }
                } else {// 回滚到原位置
                    smoothScrollTo(getScrollY(), -getScrollY(), DURATION);
                }
...
//        LogUtil.d(LOG_TAG, "mLastX= " + mLastX);
    ...
    private void smoothScrollTo(int startX, int deltaX, int duration) {
        mScroller.startScroll(0, startX, 0, deltaX, duration);
        invalidate();
    }
